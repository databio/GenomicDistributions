...
```
## Warning: package 'Biobase' was built under R version 4.0.2
```

```
## Error in value[[3L]](cond): Package 'Biobase' version 2.49.0 cannot be unloaded:
##  Error in unloadNamespace(package) : namespace 'Biobase' is imported by 'GenomicFeatures', 'SummarizedExperiment', 'ensembldb', 'AnnotationDbi' so cannot be unloaded
```
...
```
## Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'genes': object 'EnsDb.Hsapiens.v86' not found
```
...
```
## Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'seqlevels': object 'annoData' not found
```
...
```
## Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'elementMetadata': object 'annoData' not found
```
