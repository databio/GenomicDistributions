...
```
## Error in library(ChIPpeakAnno): there is no package called 'ChIPpeakAnno'
```
...
```
## Error in toGRanges(EnsDb.Hsapiens.v86, feature = "gene"): could not find function "toGRanges"
```
...
```
## Error in eval(expr, envir, enclos): object 'annoData' not found
```
...
```
## Error in sort(annoData): object 'annoData' not found
```
...
```
## Error in cutDists(dists, divisions = NULL, nbins, size, infBins): object 'TSSdist' not found
```
