...
Genomic partition distribution plots show you how your regions are distributed across genome annotation classes. This is most commonly used to show the distribution over element types, such as promoters, exons, introns, or intergenic regions. GenomicDistributions provides 3 types of partition distribution plots: *raw*, *expected*, and *cumulative*.

### Raw partition distribution plots

The most basic partition plot just provies a barplot of raw region counts. You can produce one or two-set plots like so:

```{r raw-partition-plot, fig.cap="Partition distribution plot. Raw distribution of query regions across genomic features", fig.small=TRUE}
...
```{r multiple-raw-partition-plot, fig.cap="Partition distribution plots for multiple query region sets.", fig.small=TRUE}
gp2 = calcPartitionsRef(queryList, "hg19")
plotPartitions(gp2)
```

### Percentage partition distribution plots
Additionally, Genomic Distributions offers the alternative of looking at the percentage overlap across a defined set of partitions, simply by setting the metric argument to "percent" as follows:

```{r percentage-partition-plot, fig.cap="Percentage partition distribution plot. Percentage distribution of regions across genomic features", fig.small=TRUE}
# Calculate the percentage overlaps:
perc = calcPartitionsRef(query, "hg19", metric = "percent")

# Plot the results:
plotPartitionPercents(perc)
```

As with the raw overlap partitions, you can plot more than one regionset at a time by passing the appropriate named GRangeslist:

```{r percentage-partition-plot-multi, fig.cap="Percentage partition distribution plot for multiple regionsets", fig.small=TRUE}
perc2 = calcPartitionsRef(queryList, "hg19", metric = "percent")
plotPartitionPercents(perc2)
```

When using the "percent" metric, you can also look at the percentage overlap normalized for genomic background. To do this, you need to provide a background GRanges object to the backgroundGR argument:

```{r norm-percentage-partition-plot, fig.cap="Percentage distribution plot corrected for genomic background", fig.small=TRUE}
# Create a background GRanges object:
back = GenomicRanges::shift(query, 1e8)

# Pass the background obj to the main calculation function and plot the results:
pback = calcPartitionsRef(query, "hg19", metric="percent", backgroundGR=back)
plotPartitionPercents(pback)
```

If you're assuming you can do this with more than one regionset, you're right:

```{r multiple-norm-percentage-partition-plot, fig.cap="Corrected genomic background for multiple regionsets", fig.small=TRUE}
pback = calcPartitionsRef(queryList, "hg19", metric="percent", backgroundGR=back)
plotPartitionPercents(pback)
```
...
```{r specificity-plot-single, fig.height = 6, fig.cap="Specificty of chromatin accessibility across cell types."}
...
```{r specificity-plot-multi, fig.height = 7, fig.cap="Specificty of chromatin accessibility across cell types."}
...
# Create a list of named GRanges objects
...
plotQTHist(qt1, bins=7, quantile = .015, EndBarColor = 'black',
	MiddleBarColor='darkblue')
```
