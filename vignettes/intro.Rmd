...
As well, you can bring in a set of annotations for your reference genome to annotate your query with the nearest gene, the distance to the nearest gene, and the nearest gene type:

```{r nearest-neighbor-distance-and-type-plots, fig.cap="Nearest neighbor distance annotated by type", fig.small=TRUE}
# load an annotations file
annotations = rtracklayer::import(annotationsFile)

# calculate the nearest gene annotations and plot
nearestGenes = calcNearestGenes(query, annotations)
plotNearestGenes(nearestGenes)
```
