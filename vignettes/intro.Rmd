...
Genomic partition distribution plots show you how your regions are distributed by raw numbers, relative to the expected distribution, and the cumulative distribution among promoters, exons, introns, or intergenic regions. For the raw partition distribution you can produce one or two-set plots like so:

```{r a raw partition plot}
...
```{r multiple raw partition plots in one}
gp2 = calcPartitionsRef(queryList, "hg19")
plotPartitions(gp2)
```

Here we produce one or two-set plots showing the log~10~($\frac{observed}{expected}$) of the distribution of your regions across genomic partitions:

```{r an observed/expected partition plot}
ep = calcExpectedPartitionsRef(query, "hg19")
plotExpectedPartitions(ep)
```

And can you do 2 at a time? Indeed:
```{r multiple observed/expected partition plots in one}
