...
In this plot, we see that intergenic regions reach a higher total point than intronic regions -- which is also reflected in the raw partition plot above. What we see additionally in this plot is that the intergenic spaces also cover a much larger portion of the genome, as indicated by their more more left shifted in the plot. Because of this intergenic regions and introns tend to dominate these plots. To gain an increased resolution about the behavior of other features you can simply filter out introns and intergenic features (or any other) from the `calcCumulativePartitions` output and plot the rest. Let's see how that can be done:

```{r multiple-cumulative-partition-plot-filtered, fig.cap="Cumulative partition distribution plots for multiple query region sets.", fig.small=TRUE}
cp2 = calcCumulativePartitionsRef(queryList, "hg19")
filtOut = c("intron", "intergenic")
cp2Filt = cp2[!(partition %in% filtOut),]
plotCumulativePartitions(cp2Filt)
```
