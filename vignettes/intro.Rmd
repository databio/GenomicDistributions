...
Additionally, Genomic Distributions offers the alternative of looking at the percentage overlap across a defined set of partitions simply by setting the metric="percent" argument in the above mentioned function
...
As with the raw partitions, you can plot more than one regionset at a time by creating a named list
...
When using the metric="percent" option, you can also look at the percentage overlap normalized for genomic background.
```{r norm-percentage-partition-plot, fig.cap="Percentage partition distribution plot for multiple regionsets correcting for genomic background", fig.small=TRUE}
backGR = GenomicRanges::shift(query, 1e8)
pback = calcPartitionsRef(queryList, "hg19", metric = "percent", backgroundGR = backGR)
